...
if __name__ == '__main__':
    ...
    if mode=="ranking":
        ...
        for target in list_targets:
            ...
            # Calculate full RMSD
            ranked_ligands_rmsd={}
            ranked_ligands_rmsd_ecr={}
            for ligand in list_ligands:
                try:
                    rmsd_m,rmsd_e=calculate_rmsd.calculate_mean_RMSD("temp_ranking",ligand,list_software,sigma)
                except:
                    rmsd_m=10.0
                    rmsd_e=10.0
                ranked_ligands_rmsd[ligand]=rmsd_m
                ranked_ligands_rmsd_ecr[ligand]=rmsd_e
            ...
            # Save rank RMSD ECR
            sorted_x = sorted(ranked_ligands_rmsd_ecr.items(), key=operator.itemgetter(1))

            rank_file_rmsd=open("ranks/rank_rmsd_ecr_{}.txt".format(target),"w")
            ecr_rmsd_ecr[target]={}
            for j,element in enumerate(sorted_x):
                rank_file_rmsd.write("{}\t{}\t{}\n".format(j+1,element[0],element[1]))
                ecr_rmsd_ecr[target][element[0]]=j+1
            rank_file_rmsd.close()
